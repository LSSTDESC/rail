...
A key concept in `rail` are ceci `Pipelines`, which run a series of `RailStages` using
the `ceci` framework.
...
Subclasses can build particular types of analysis piplines subject to some configuration
choices, such as which algorithms to use.
